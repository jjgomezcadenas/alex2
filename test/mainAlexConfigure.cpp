...
int main(int argc, char **argv)
{
  ...
  string path ="/Users/jjgomezcadenas/Development/devnext/alex2/example/";
  string alexConf="AlexConfig.xml"

  string pathAlex = PathFromStrings(path,alexConf);
  klog << log4cpp::Priority::INFO << "Path to alex conf";
  klog << log4cpp::Priority::INFO << pathAlex;
	AlexConfigure::Instance().ParseConfiguration(pathAlex);
	...
	string aConfHeader="AConf.h"
  string pathAConfHeader = PathFromStrings(path,aConfHeader)
